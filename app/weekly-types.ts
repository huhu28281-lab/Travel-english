...
export function courseWeekStart(index:number) {
  if (!Number.isInteger(index) || index < 0 || index > 3800) throw new Error("Invalid course week");
  return new Date(EPOCH + index * WEEK + OFFSET).toISOString().slice(0,10);
}
