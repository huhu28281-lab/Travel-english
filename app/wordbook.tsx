...
export default function Wordbook({book,rate,onPractice}:{book:ReturnType<typeof useVocabulary>;rate:number;onPractice:(text:string)=>void}) {
  ...
  return <div className="feature-page">
    <div className="page-heading"><div><p className="eyebrow">WORDS THAT WORK FOR YOU</p><h1>단어장</h1><p className="heading-sub">이번 주 여행 표현과 내가 담은 문장을 복습해요.</p></div><button className="primary-button" onClick={()=>{setDraft(emptyWord);setOpen(true);}}><Plus size={18}/>직접 추가</button></div>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="feature-dialog"><DialogHeader><DialogTitle>나만의 표현 추가</DialogTitle><DialogDescription>다음 여행에서 쓰고 싶은 영어를 담아보세요.</DialogDescription></DialogHeader><form className="feature-form" onSubmit={async e=>{e.preventDefault();const existing=book.words.find(w=>wordKey(w.english)===wordKey(draft.english));if(await book.save({...draft,known:existing?.known||false,saved:true}))setOpen(false);}}><label htmlFor="word-english">영어 단어 또는 표현<Input id="word-english" value={draft.english} required maxLength={700} onChange={e=>setDraft({...draft,english:e.target.value})} placeholder="boarding pass"/></label><label htmlFor="word-meaning">한국어 뜻<Input id="word-meaning" value={draft.meaning} required maxLength={700} onChange={e=>setDraft({...draft,meaning:e.target.value})} placeholder="탑승권"/></label><label htmlFor="word-example">예문 <span className="muted">선택</span><Textarea id="word-example" value={draft.example} maxLength={700} onChange={e=>setDraft({...draft,example:e.target.value})} rows={2}/></label><label htmlFor="word-example-ko">예문 뜻 <span className="muted">선택</span><Input id="word-example-ko" value={draft.exampleKo} maxLength={700} onChange={e=>setDraft({...draft,exampleKo:e.target.value})}/></label>{book.error && <p className="form-error" role="alert">{book.error}</p>}<button className="primary-button" type="submit" disabled={book.busy || !book.ready}>{book.busy?<Loader2 className="spin" size={17}/>:<Bookmark size={17}/>}단어장에 저장</button></form></DialogContent></Dialog>
  </div>;
}
