...
export function useVocabulary(weeklyWords:Word[]=starterWords) {
  ...
  const reload=useCallback(async()=>{
    ...
    try {
      const response=await learnerFetch("/api/vocabulary",{cache:"no-store"}),data=await response.json() as {words:Word[];word:Word;error?:string};
  ...
  const save=useCallback(async(word:Word)=>{
    ...
    try {
      const response=await learnerFetch("/api/vocabulary",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(word)}),data=await response.json() as {words:Word[];word:Word;error?:string};
