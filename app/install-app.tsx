...
export default function InstallApp({browserScoped = false}:{browserScoped?:boolean}) {
  ...
  const install = async () => {
    ...
    try {
      ...
      if (choice.outcome === "accepted") {
        setMessage("설치를 요청했어요. 완료되면 홈 화면이나 앱 목록에서 ‘트래블 잉글리시’를 찾아주세요.");
      } else {
  ...
  return <>
    <button className="install-trigger" onClick={()=>void install()} disabled={busy} aria-label="트래블 잉글리시를 홈 화면에 추가"><Download size={17}/><span>{busy ? "설치 준비 중" : "홈 화면에 추가"}</span></button>
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="install-dialog" showCloseButton={false}>
        ...
        <DialogHeader className="install-heading">
          <img src="/icons/icon-192.png" width="64" height="64" alt="트래블 잉글리시 비행기 아이콘"/>
          <DialogTitle>내 홈 화면에 트래블 잉글리시</DialogTitle>
